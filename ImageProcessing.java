...
public class ImageProcessing {
    ...
    for (int i = 0; i<imageTwoD.length; i++){
      for (int j = 0; j<imageTwoD[i].length; j++){
        ...
        rgba[2] = 255 - rgba[2];
        rgba[3] = 255 - rgba[3];
        inverted[i][j] = getColorIntValFromRGBA(rgba);
      }
    }
    ...
    int[][] expanded = new int[imageTwoD.length][imageTwoD[0].length*2];
    int it = 0;
    for (int i = 0; i<imageTwoD.length; i++){
      for (int j = 0; j<imageTwoD[i].length; j++){
        it = j*2;
        expanded[i][it] = imageTwoD[i][j];
        expanded[i][it+1] = imageTwoD[i][j];
      }
    }
		return expanded;
	}
    ...
    int[][] updown = new int[imageTwoD.length][imageTwoD[0].length];
    for (int i = 0; i<imageTwoD.length; i++){
      for (int j = 0; j<imageTwoD[i].length; j++){
        updown[i][j] = imageTwoD[(imageTwoD.length-1)-i][(imageTwoD[i].length-1)-j];
      }
    }
		return updown;
	}
    ...
    for (int i = 0; i<canvas.length;i++){
      for (int j = 0; j<canvas[i].length;j++){
        int randColPix1 = rand.nextInt(256);
        int randColPix2 = rand.nextInt(256);
        int randColPix3 = rand.nextInt(256);
        int[] rgba = {randColPix1, randColPix2, randColPix3, 255};
